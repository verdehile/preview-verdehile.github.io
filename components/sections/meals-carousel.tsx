...
const mealsData = [
  {
    ...
    description: "Perfectly seasoned grilled chicken with brown rice and seasonal vegetables",
    macro: "P: 42g | C: 48g | F: 8g",
  },
  {
    id: 2,
    name: "Buddha Bowl Deluxe",
    description: "Chickpeas, sweet potato, kale, and tahini dressing for complete nutrition",
    macro: "P: 15g | C: 52g | F: 14g",
  },
  {
    id: 3,
    name: "Turkey Meatballs",
    description: "Lean ground turkey meatballs with zucchini noodles and marinara",
...
export default function MealsCarousel() {
  ...
  return (
    <section id="meals" className="py-20 px-4 sm:px-6 lg:px-8 bg-secondary/30" ref={ref}>
      <div className="max-w-6xl mx-auto">
        <div
          className="opacity-0 animate-fade-in"
          style={{ animation: isVisible ? "fadeIn 0.6s ease-out forwards" : "none" }}
        >
          <h2 className="text-4xl font-heading font-bold text-center mb-4 text-foreground">A Glimpse of Our Meals</h2>
          <p className="text-center text-foreground/70 mb-12 max-w-2xl mx-auto">
            Explore our diverse menu of healthy, delicious meals crafted to keep you on track with your fitness goals.
          </p>
        </div>
                ...
                {[...Array(3)].map((_, i) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ animation: `scaleIn 0.6s ease-out ${i * 0.1}s forwards` }}
                    >
                      <div className="w-full h-40 bg-muted rounded-lg flex items-center justify-center hover:bg-muted/80 transition">
                        <span className="text-muted-foreground text-sm">Meal image</span>
                      </div>
                      <h3 className="font-heading font-bold text-lg text-foreground mt-4">{meal.name}</h3>
                      <p className="text-sm text-foreground/70 mt-2">{meal.description}</p>
                      <p className="text-xs font-mono text-primary bg-primary/5 p-2 rounded mt-4">{meal.macro}</p>
                    </div>
                  )
                })}
              </div>
          ...
          <div className="flex justify-center gap-2 mt-8">
            {mealsData.map((_, index) => (
              <button
                ...
                className={`h-2 rounded-full transition ${index === currentIndex ? "bg-primary w-8" : "bg-border w-2"}`}
